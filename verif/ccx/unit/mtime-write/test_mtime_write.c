...
int test_main() {
    ...
    croyde_csp_mtimecmp[0] = new_mtimecmp_value;

    if(croyde_csp_rd_mtimecmp() != new_mtimecmp_value) {
        return 6;
    }

    for(int i = 0; i < 10; i ++) {

        new_mtimecmp_value = croyde_csp_rd_mtime() * 20;
    ...
    croyde_csp_mtimecmp[0] = -1;

    // We should be able to read/write mtime
    uint64_t new_mtime_value = 0xF000000000000000;

    croyde_csp_mtime[0] = new_mtime_value;
    
    uint64_t nxt_mtime_value = croyde_csp_rd_mtime() - new_mtime_value;
