...
extern void do_wfi_store(volatile uint64_t * addr, uint64_t val);

uint64_t val_to_store = 0xdeadbeefbadf00d5;
volatile uint64_t store_target = 0;
...
void c_trap_handler() {
    ...
    if(store_target  ==  val_to_store    ) {__putstr("F\n"); test_fail();}
